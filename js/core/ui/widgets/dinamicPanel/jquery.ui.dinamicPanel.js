                            ...
                            if (type == 'identify'){
								...
								if (typeof(item.value) == 'string' &&  item.value.indexOf('|') >= 0){ //es un campo especial
									...
									if (values.length > 2){
                                        ...
                                        item.value = (item.value.search('http') > -1)?'<a href="'+item.value.split('|')[1]+ '" target="_blank" onclick="window.open(this.href, this.target,\'width=600, height=400, menubar=no\');return false">'+item.value.split('|')[0]+'</a>':item.value;
