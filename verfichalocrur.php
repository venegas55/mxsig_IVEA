...
<div id="tabs" style="font-size:10px">
  ...
  <div id="tabs-1" class='panel'>
	...
	pob81 as\"Población masculina de 70 años y más\" FROM datosestadisticos.locrur_poblacion where cvegeo = '$geografico'";
	//echo $query;
  ...
  <div id="tabs-2" class='panel'>
	<?php
	$query = "SELECT cvegeo as\"Clave Localidad\", grado_marg as\"Grado de Marginación CONAPO\", gra_re_soc \"Grado de Rezago Social CONEVAL\" FROM datosestadisticos.locrur_desarrollo_social where cvegeo = '$geografico'";
	//echo $query;
	$result = pg_query($query) or die('Query failed: ' . pg_last_error());
	$i = pg_num_fields($result);
	if ($row = pg_fetch_array($result)){
